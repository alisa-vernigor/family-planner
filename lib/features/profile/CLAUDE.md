...
- **pages/profile_settings_page.dart** — `ProfileSettingsPage(profileId, picker?)`: редактирование имени, био, загрузка/удаление аватара через `ImagePicker`. **`picker`** — `@visibleForTesting` инжектируемый `ImagePicker` (для тестов, без реальной камеры/галереи; `null` → создаётся `ImagePicker()`). Превью pending-изменений. Кнопка «Сохранить» появляется только при наличии изменений. `contentType` при загрузке аватара: приоритет — `XFile.mimeType` (web), фолбэк — расширение `XFile.name` (нативные платформы). Сохранение: `_saveProfile` проверяет `cubit.state is ProfileFailure` после `updateProfile` и в этом случае бросает `_SaveProfileFailedException` (перехватывается `catch`), чтобы не показывать ложный «Профиль сохранён.» и не сбрасывать pending-изменения — cubit ловит ошибки репозитория внутри себя и не пробрасывает их наружу.
