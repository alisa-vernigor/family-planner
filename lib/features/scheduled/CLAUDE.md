...
- **widgets/calendar_view.dart** — `CalendarView`: календарный режим через пакет `table_calendar`.
  ...
  - Чекбокс выполнения в карточке дня (`_CalendarDayTaskCard`) гейтится через `canBeCompletedBy(currentMemberId)`: не в allowedMemberIds → неактивен (тултип «Вы не назначены исполнителем»). `currentMemberId` прокидывается и в `TimeScaleView`.
...
- **widgets/time_scale_view.dart** — `TimeScaleView` (StatefulWidget): временная шкала задач (неделя/день) с часами, как события Google Calendar.
  ...
  - Принимает `currentMemberId` — используется для гейта чекбокса выполнения (`_TimelineTaskBlock`): если `!task.canBeCompletedBy(currentMemberId)` — чекбокс неактивен (тултип «Вы не назначены исполнителем», как в `TaskCard`).
